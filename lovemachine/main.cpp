...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	HWND hwnd = CreateWindow(
		wc.lpszClassName,
		_T("lovemachine_css - ImGui Overlay & UI Preview"),
		WS_OVERLAPPEDWINDOW,
		100, 100, 1280, 800,
	...
	// Initialize Config System & Settings Bindings
	configs::on_inject();
	...
	// Setup ImGui Win32 & DX9 backends
	ImGui_ImplWin32_Init(hwnd);
	ImGui_ImplDX9_Init(g_pd3dDevice);

	// Setup custom dark style
	Menu::SetupStyle();
	...
	while (!done)
	{
		...
		// Handle ESC to exit if menu is hidden or pressed explicitly
		static bool esc_was_down = false;
		bool esc_is_down = (GetAsyncKeyState(VK_ESCAPE) & 0x8000) != 0;
		if (esc_is_down && !esc_was_down && !Menu::show_menu)
		{
			done = true;
		}
		esc_was_down = esc_is_down;

		// Handle D3D device loss / reset
		...
		// End Frame & Render DirectX 9 Scene
		ImGui::EndFrame();
		g_pd3dDevice->SetRenderState(D3DRS_ZENABLE, FALSE);
		g_pd3dDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
		g_pd3dDevice->SetRenderState(D3DRS_SCISSORTESTENABLE, FALSE);
		
		D3DCOLOR clear_col = D3DCOLOR_RGBA(18, 18, 22, 255);
