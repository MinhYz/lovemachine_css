...
namespace AccessoriesMesh
{
	...
	// 1. Authentic Asian Conical Rice Hat (Shallow Organic Bamboo Weave & Concentric Rings)
	inline void GenerateNonLa(float hat_size, float hat_height, std::vector<Vertex3D>& out_vertices, std::vector<Triangle3D>& out_triangles)
	{
		out_vertices.clear();
		out_triangles.clear();
		...
		for (int i = 0; i < segments; i++)
		{
			...
			float vz = h_mid;
			out_vertices.push_back({ vx, vy, vz, std::cos(angle) * 0.7f, std::sin(angle) * 0.7f, 0.7f, (float)i / (float)segments, 0.5f });
		}
		...
		for (int i = 0; i < segments; i++)
		{
			...
			float vz = -1.5f;
			out_vertices.push_back({ vx, vy, vz, std::cos(angle) * 0.8f, std::sin(angle) * 0.8f, 0.4f, (float)i / (float)segments, 1.0f });
		}
	...
	inline void GenerateHalo(float radius, float height_offset, float float_time, std::vector<Vertex3D>& out_vertices, std::vector<Triangle3D>& out_triangles)
	{
		out_vertices.clear();
		out_triangles.clear();
		...
		float r_minor = 1.8f;
		float float_z = 10.0f + height_offset * 0.4f + std::sin(float_time * 2.5f) * 1.5f;
		...
		for (int i = 0; i < ring_pts; i++)
		{
			...
			for (int j = 0; j < tube_pts; j++)
			{
				...
				out_vertices.push_back({ vx, vy, vz, std::cos(u) * std::cos(v), std::sin(u) * std::cos(v), std::sin(v), (float)i / (float)ring_pts, 0 });
			}
		}
		...
		for (int i = 0; i < ring_pts; i++)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				...
				out_vertices.push_back({ vx, vy, vz, std::cos(u), std::sin(u), 1.0f, 0, 0 });
			}
		}
	...
	// 3. Majestic 3D Devil Horns (Curved Gothic Ram Horns with Smooth Segments & Ribs)
	inline void GenerateDevilHorns(float scale, std::vector<Vertex3D>& out_vertices, std::vector<Triangle3D>& out_triangles)
	{
		out_vertices.clear();
		out_triangles.clear();
		...
		for (int side = -1; side <= 1; side += 2)
		{
			...
			for (int r = 0; r < num_rings; r++)
			{
				...
				for (int p = 0; p < pts_per_ring; p++)
				{
					...
					out_vertices.push_back({ px, py, pz, std::cos(angle), std::sin(angle), 0.3f, frac, (float)p / (float)pts_per_ring });
				}
			}
			...
			for (int p = 0; p < pts_per_ring; p++)
			{
				...
				uint16_t i1 = (uint16_t)(base_idx + (num_rings - 1) * pts_per_ring + next_p);
	...
	inline void GenerateCrown(float size, float height, std::vector<Vertex3D>& out_vertices, std::vector<Triangle3D>& out_triangles)
	{
		out_vertices.clear();
		out_triangles.clear();
		...
		for (int i = 0; i < spires * 2; i++)
		{
			...
			out_vertices.push_back({ std::cos(a) * r_base, std::sin(a) * r_base, 1.0f * sc, std::cos(a), std::sin(a), 0, 0, 0 });
			out_vertices.push_back({ std::cos(a) * (r_base * 1.05f), std::sin(a) * (r_base * 1.05f), (1.0f + h_base), std::cos(a), std::sin(a), 0, 0, 0 });
		}
		...
		for (int i = 0; i < spires; i++)
		{
			...
			out_vertices.push_back({ px, py, pz, std::cos(a), std::sin(a), 1.0f, 0.5f, 1.0f });
	...
	inline void GenerateCatEars(float scale, std::vector<Vertex3D>& out_vertices, std::vector<Triangle3D>& out_triangles)
	{
		out_vertices.clear();
		out_triangles.clear();
		...
		for (int side = -1; side <= 1; side += 2)
		{
			...
			out_vertices.push_back({ s * 7.5f * sc, -1.5f * sc, 3.5f * sc, s * 0.7f, -0.3f, 0.6f, 0.0f, 0.0f });
			out_vertices.push_back({ s * 2.8f * sc, 1.0f * sc, 4.5f * sc, s * 0.2f, 0.2f, 0.9f, 1.0f, 0.0f });
			out_vertices.push_back({ s * 5.2f * sc, 3.2f * sc, 3.8f * sc, 0.0f, 0.9f, 0.4f, 0.5f, 0.0f });
			out_vertices.push_back({ s * 6.8f * sc, -1.0f * sc, 13.2f * sc, s * 0.5f, -0.2f, 0.8f, 0.5f, 1.0f });
			out_vertices.push_back({ s * 5.0f * sc, 1.8f * sc, 6.5f * sc, 0.0f, 0.9f, 0.4f, 0.5f, 0.5f });

			out_triangles.push_back({ (uint16_t)(b + 0), (uint16_t)(b + 3), (uint16_t)(b + 1), 1.0f, 0 });
			out_triangles.push_back({ (uint16_t)(b + 0), (uint16_t)(b + 2), (uint16_t)(b + 3), 1.0f, 0 });

			out_triangles.push_back({ (uint16_t)(b + 1), (uint16_t)(b + 3), (uint16_t)(b + 4), 1.0f, 0xFFFFA0C0 });
			out_triangles.push_back({ (uint16_t)(b + 1), (uint16_t)(b + 4), (uint16_t)(b + 2), 1.0f, 0xFFFF85A5 });
			out_triangles.push_back({ (uint16_t)(b + 2), (uint16_t)(b + 4), (uint16_t)(b + 3), 1.0f, 0xFFFFB5D0 });
		}
	}
}
