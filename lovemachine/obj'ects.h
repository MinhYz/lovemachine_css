...
namespace obj
{
	...
	void create_tabs()
	{
		for (int tab_id = legit; tab_id < tab_max; tab_id++)
		{
			...
			prim::filled_box(x, sets->menu.y, x1, y, tab); // îñíîâà
		...
		void on_draw()
		{
			...
			for (int value_id = 0; value_id < size; value_id++)
			{
				...
				if ((!selectable && value_id == *value) || (selectable && values[value_id]))
					vtext = color::enabled() + 65; // öâåò ñòðàííî ðàáîòàåò
	...
	//TODO : ïîôèêñèòü îòðèñîâêó ëèíèé è òä è ñäåëàòü íîðìàëüíûé êóðñîð
	void draw_mouse()
	{
