...
namespace offsets
{
	...
	const char* initialize_tables()
	{
		...
		auto p_class = p_client->get_all_classes(); // âñå êëèåíòñêèå êëàññû
		if (!p_class)
			return "/fail/ failed to get classes";

		while (p_class)
		{
			auto table = p_class->recv_table; // òàáëèöà êëàññà
			tables.push_back(table); // äîáàâëÿåì â ñïèñîê òàáëèöó

			p_class = p_class->next; // ïåðåõîäèì ê ñëåäóþùåìó êëàññó
		}

		return "/tables/ succesfull";
	}

	//TODO : ïîíÿòü êàê ðàáîòàåò ýòà ðåêóðñèâíàÿ ôóíêöèÿ
